...
int main(int argc, char ** argv)
{
  ...
  long long mul_time;
  int a_dim1, a_dim2, b_dim1, b_dim2, rank , size;
  ...
  //assign spaces in processor based on matrix size
  MPI_Barrier(MPI_COMM_WORLD);
  MPI_Scatter(A, (a_dim1*a_dim2)/size, MPI_INT, C, (a_dim2*b_dim2)/size, MPI_INT, 0,MPI_COMM_WORLD);
  MPI_Bcast(B, b_dim1*b_dim2, MPI_INT, 0, MPI_COMM_WORLD);
  
  /* perform matrix multiplication */
  team_matmul(A, B, C, a_dim1, a_dim2, b_dim2);

  //get messages back from separate processes
  MPI_Gather(C, (a_dim2*b_dim2)/size, MPI_INT, C, (a_dim2*b_dim2)/size, MPI_INT, 0,MPI_COMM_WORLD);
  MPI_Finalize();
